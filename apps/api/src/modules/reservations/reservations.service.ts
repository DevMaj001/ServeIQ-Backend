...
export class ReservationsService {
  ...
  private generateConfirmationCode(): string {
    // CSPRNG: Math.random()'s V8 state is recoverable from observed codes,
    // and these codes authorize unauthenticated cancel/lookup (with PII).
  ...
  private async validateBranchConfig(
    ...
    if ((options?.requireEnabled ?? true) && !config.enabled) {
      throw new BadRequestException(
        'Reservations are not enabled for this branch',
      );
    }
  ...
  private async createTabFromReservation(
  ...
  private async checkAvailability(
    ...
    const overlapping = existing.filter(
      (r) =>
        ...
        new Date(r.reservation_time).getTime() +
          r.duration_minutes * 60 * 1000 >
          slotStart.getTime(),
    );
  ...
  async create(
    ...
    if (
      !config.allow_online &&
      (dto.source ?? ReservationSource.PUBLIC) === ReservationSource.PUBLIC
    ) {
      throw new BadRequestException(
        'Online reservations are not enabled for this branch',
      );
    }
    ...
    const maxAdvance = new Date(
      now.getTime() + config.advance_days * 24 * 60 * 60 * 1000,
    );
    if (reservationTime > maxAdvance) {
      throw new BadRequestException(
        `Cannot book more than ${config.advance_days} days in advance`,
      );
    }
    ...
    // Validate party size
    if (
      dto.party_size < config.min_party_size ||
      dto.party_size > config.max_party_size
    ) {
      throw new BadRequestException(
        `Party size must be between ${config.min_party_size} and ${config.max_party_size}`,
      );
    }
    ...
    const branch = await this.branchRepo.findOne({ where: { id: branchId } });
    if (!branch) throw new NotFoundException('Branch not found');
    const initialStatus = config.auto_confirm
      ? ReservationStatus.CONFIRMED
      : ReservationStatus.PENDING;
  ...
  async createWalkin(
    ...
    if (dto.table_id) {
      table = await this.tableRepo.findOne({
        where: { id: dto.table_id, branch_id: branchId },
      });
      if (!table) throw new NotFoundException('Table not found');
      if (table.capacity < dto.party_size)
        throw new BadRequestException('Table too small for party size');
    }
    ...
    if (table) {
      const conflict = overlapping.some((r) => r.table_id === table!.id);
      if (conflict)
        throw new ConflictException('Table is not available right now');
    } else {
      // Find any available table
      const suitableTables = await this.tableRepo.find({
        where: { branch_id: branchId },
      });
      const suitable = suitableTables.filter(
        (t) => t.capacity >= dto.party_size,
      );
      ...
      if (!table)
        throw new ConflictException('No tables available for walk-in');
    }
  ...
  async findAll(
    ...
    user: { businessId: string; branchId: string; role: string },
  ): Promise<ReservationView[]> {
    ...
    if (query.from || query.to) {
      if (query.from && query.to) {
        where.reservation_time = Between(
          new Date(query.from),
          new Date(query.to),
        );
      } else if (query.from) {
    ...
    const tables = tableIds.length
      ? await this.tableRepo.findBy({ id: In(tableIds) })
      : [];
  ...
  async findById(id: string, businessId: string): Promise<ReservationView> {
    ...
    const table = reservation.table_id
      ? await this.tableRepo.findOne({ where: { id: reservation.table_id } })
      : null;
  ...
  async update(
    ...
    if (dto.reservation_time || dto.party_size || dto.duration_minutes) {
      const newTime = dto.reservation_time
      ...
      const maxAdvance = new Date(
        new Date().getTime() + config.advance_days * 24 * 60 * 60 * 1000,
      );
      if (newTime > maxAdvance)
        throw new BadRequestException(
          `Cannot book more than ${config.advance_days} days in advance`,
        );
      if (
        newPartySize < config.min_party_size ||
        newPartySize > config.max_party_size
      ) {
        throw new BadRequestException(
          `Party size must be between ${config.min_party_size} and ${config.max_party_size}`,
        );
      ...
      if (!availability)
        throw new ConflictException(
          'No tables available for the new time/party size',
        );
    ...
    if (dto.customer_email !== undefined)
      reservation.customer_email = dto.customer_email;
    ...
    if (dto.duration_minutes)
      reservation.duration_minutes = dto.duration_minutes;
    ...
    if (dto.status && dto.status !== reservation.status) {
      ...
      if (dto.status === ReservationStatus.CANCELLED) {
        ...
        // Auto-create tab
        const existingTab = await this.tabRepo.findOne({
  ...
  async seatWalkin(
    ...
    const reservation = await this.reservationRepo.findOne({
      where: { id: reservationId, branch_id: branchId },
    });
    if (!reservation) throw new NotFoundException('Reservation not found');
    if (
      reservation.status !== ReservationStatus.CONFIRMED &&
      ...
      throw new BadRequestException(
        'Only pending/confirmed reservations can be seated',
      );
    }
  ...
  async getTodaySummary(branchId: string): Promise<{
    ...
    const reservations = await this.reservationRepo.find({
      where: {
        branch_id: branchId,
        reservation_time: Between(today, tomorrow),
      },
    });
    ...
    return {
      ...
      walkins: reservations.filter((r) => r.source === ReservationSource.WALKIN)
        .length,
    };
  }
  ...
  async sendReminders(): Promise<number> {
    ...
    const reminderWindow = new Date(
      Date.now() + configReminderWindow * 60 * 1000,
    );
    const now = new Date();
  ...
  private async emitReservationUpdate(
    branchId: string,
    reservation: Reservation,
  ): Promise<void> {
