...
export class OrderController {
  ...
  @ApiResponse({
    status: 201,
    description: 'Order items added to tab.',
    type: [Order],
  })
  ...
  @ApiOperation({
    summary:
      'Get pending approval orders (paginated). Waiters see only their own; supervisors/managers/owners see all.',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'List of order items for the tab.',
    type: [Order],
  })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  ...
  @ApiOperation({
    summary: 'Update order item (Supervisor/Manager/Owner only)',
  })
  @ApiParam({ name: 'id', description: 'Order item UUID' })
  ...
  @Roles(UserRole.SUPERVISOR, UserRole.OWNER, UserRole.MANAGER)
  @ApiOperation({
    summary: 'Remove order item (Supervisor/Manager/Owner only)',
  })
  @ApiParam({ name: 'id', description: 'Order item UUID' })
  ...
  @ApiOperation({
    summary:
      'Approve a pending order (Supervisor/Owner only) — requires department + prep time',
  })
  @ApiParam({ name: 'id', description: 'Order item UUID' })
  @ApiBody({ type: ApproveOrderDto })
  @ApiResponse({
    status: 200,
    description: 'Order approved and timer started.',
  })
  ...
  @Roles(UserRole.SUPERVISOR, UserRole.OWNER, UserRole.MANAGER)
  @ApiOperation({
    summary: 'Decline a pending order (Supervisor only) — requires reason',
  })
  @ApiParam({ name: 'id', description: 'Order item UUID' })
  ...
  @ApiOperation({
    summary:
      'Confirm waiter has picked up the order (Owner/Manager/Supervisor)',
  })
  @ApiParam({ name: 'id', description: 'Order item UUID' })
  @ApiResponse({
    status: 200,
  ...
  @Roles(UserRole.OWNER, UserRole.MANAGER, UserRole.SUPERVISOR, UserRole.WAITER)
  @ApiOperation({
    summary: 'Confirm delivery (Owner/Manager/Supervisor/Waiter)',
  })
  @ApiParam({ name: 'id', description: 'Order item UUID' })
