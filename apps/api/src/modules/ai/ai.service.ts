...
It serves two apps: Admin (web) and Waiter (mobile/tablet).`;
  ...
  async getSalesReport(branchId: string, question: string, dateFrom?: string, dateTo?: string): Promise<string> {
    const client = await this.getClient();
    if (!client) {
      throw new Error('AI service unavailable: NEMOTRON_API_KEY not configured');
    }
    ...
    if (tabIds.length > 0) {
      ...
      for (const item of sorted) {
        const menuItem = await this.menuItemRepo.findOne({ where: { id: item.menu_item_id } });
        topItems.push({
    ...
    const dataContext = JSON.stringify({
      period: { from, to },
      revenue: { total_kobo: totalRevenue, transaction_count: bills.length, average_bill_kobo: bills.length > 0 ? Math.round(totalRevenue / bills.length) : 0 },
    ...
    const response = await client.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a restaurant operations analyst. Answer the user\'s question using ONLY the provided data. Be concise, specific, and actionable. Use kobo amounts — convert to naira for readability (e.g. 15000 kobo = ₦150). If the data doesn\'t contain enough info to answer fully, say so and summarize what you can.',
        },
        {
          role: 'user',
  ...
  async getWastageInsights(branchId: string): Promise<string> {
    ...
    const wastageByItem: Record<string, { name: string; total_lost: number; entries: number; estimated_cost_kobo: number }> = {};
    for (const adj of adjustments) {
      const item = trackedItems.find(i => i.id === adj.menu_item_id);
      if (!item) continue;
      if (!wastageByItem[adj.menu_item_id]) {
        wastageByItem[adj.menu_item_id] = { name: item.name, total_lost: 0, entries: 0, estimated_cost_kobo: 0 };
      }
      ...
      if (item.cost_price_kobo) {
        wastageByItem[adj.menu_item_id].estimated_cost_kobo += Math.abs(Number(adj.quantity_change)) * item.cost_price_kobo;
      }
    }

    const lowStockItems = trackedItems
      .filter(i => Number(i.quantity_in_stock) <= Number(i.reorder_level))
      .map(i => ({ name: i.name, stock: Number(i.quantity_in_stock), reorder_level: Number(i.reorder_level), cost_price_kobo: i.cost_price_kobo }));

    const dataContext = JSON.stringify({
      tracked_item_count: trackedItems.length,
      wastage_adjustments_last_30_days: Object.values(wastageByItem).sort((a: any, b: any) => b.total_lost - a.total_lost),
    ...
    const response = await client.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an inventory efficiency expert for restaurants. Analyze wastage patterns and give actionable advice. Highlight the top wastage items, estimate financial impact, and suggest specific fixes (e.g. portion control, supplier issues, menu rebalancing). Be concise and practical.',
        },
        {
          role: 'user',
  ...
  async getRestockRecommendations(branchId: string): Promise<string> {
    ...
    const lowItems = trackedItems.filter(i => Number(i.quantity_in_stock) <= Number(i.reorder_level));
    ...
    for (const p of purchases) {
      usageByItem[p.menu_item_id] = (usageByItem[p.menu_item_id] || 0) + Number(p.quantity_change);
    }
    ...
    const recommendations = lowItems.map(item => {
      ...
      const daysRemaining = dailyRate > 0 ? Math.round(currentStock / dailyRate) : 999;
      return {
        name: item.name,
        ...
        suggested_order_qty: dailyRate > 0 ? Math.ceil(dailyRate * 14) : Math.ceil(Number(item.reorder_level) * 2),
      };
    });

    const healthyItems = trackedItems
      .filter(i => Number(i.quantity_in_stock) > Number(i.reorder_level))
      .map(i => ({ name: i.name, stock: Number(i.quantity_in_stock), reorder_level: Number(i.reorder_level) }));

    const dataContext = JSON.stringify({
      total_tracked: trackedItems.length,
      items_needing_restock: recommendations.sort((a, b) => a.days_until_depleted - b.days_until_depleted),
    ...
    const response = await client.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a restaurant supply chain advisor. Review the inventory data and give a prioritized restock plan. Highlight urgent items (less than 3 days of stock), suggest order quantities, and flag items with no assigned supplier. Be concise and actionable.',
        },
        {
          role: 'user',
