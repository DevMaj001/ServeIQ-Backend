...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(payload: any) {
    if (payload.pin_token_version !== undefined || payload.staff_token_version !== undefined) {
      const user = await this.dataSource.getRepository(User).findOne({ where: { id: payload.sub } });
      if (!user) throw new UnauthorizedException('User not found');

      if (payload.pin_token_version !== undefined && user.pin_token_version !== payload.pin_token_version) {
      ...
      if (payload.staff_token_version !== undefined) {
        ...
        if (!branch || branch.staff_token_version !== payload.staff_token_version) {
          throw new UnauthorizedException('Token invalidated — staff session expired');
        }
      }
    }
