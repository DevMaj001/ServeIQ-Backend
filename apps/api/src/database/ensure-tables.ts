...
export async function ensureTables(ds: DataSource) {
  const hasModifierGroups = await ds
    .query(
      `SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'modifier_groups')`,
    )
    .then((r) => r[0]?.exists);

  // Guard: tabs table must exist before altering it
  const hasTabs = await ds
    .query(
      `SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'tabs')`,
    )
    .then((r) => r[0]?.exists);
  ...
  console.log(
    '[ensureTables] Running deferred DDL for modifiers, splits, sync, printing...',
  );
