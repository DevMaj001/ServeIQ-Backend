...
async function seed() {
  ...
  const permissionEntities = permissionRepo.create(
    allPermissions.map(p => ({ code: p.code, name: p.name, description: p.description, category: p.category }))
  );
  ...
  // Create Menu Items
  const menuItems = [
    { name: 'Jollof Rice', category: 'Food', price_kobo: 5000, unit: 'plate', sku: 'JOL-001', image_url: '', is_available: true },
    { name: 'Fried Rice', category: 'Food', price_kobo: 4500, unit: 'plate', sku: 'FRI-001', image_url: '', is_available: true },
    { name: 'Pounded Yam & Egusi', category: 'Food', price_kobo: 6000, unit: 'plate', sku: 'PYA-001', image_url: '', is_available: true },
    { name: 'Chicken Suya', category: 'Food', price_kobo: 3500, unit: 'skewer', sku: 'SUY-001', image_url: '', is_available: true },
    { name: 'Coca Cola', category: 'Drinks', price_kobo: 800, unit: 'bottle', sku: 'COC-001', image_url: '', is_available: true },
    { name: 'Bottled Water', category: 'Drinks', price_kobo: 500, unit: 'bottle', sku: 'WAT-001', image_url: '', is_available: true },
    { name: 'Chapman', category: 'Drinks', price_kobo: 1500, unit: 'glass', sku: 'CHA-001', image_url: '', is_available: true },
    { name: 'Ice Cream', category: 'Desserts', price_kobo: 2000, unit: 'scoop', sku: 'ICE-001', image_url: '', is_available: true },
  ];
  ...
  const sampleTab = tabRepo.create({
    ...
    waiter_id: (await userRepo.findOne({ where: { email: 'waiter@demo.com' } }))?.id,
