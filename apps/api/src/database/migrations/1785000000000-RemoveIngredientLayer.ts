...
export class RemoveIngredientLayer1785000000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        console.log(`Bucket A (stock-tracked, linked to ingredient): ${bucketA[0]?.count ?? 0}`);
        console.log(`Bucket B (recipe-only, non-tracked): ${bucketB[0]?.count ?? 0}`);
        console.log(`Bucket C (neither, non-tracked): `);
        console.log(`Affected Bucket A items:`, JSON.stringify(bucketAAffected[0]?.items ?? []));
        console.log(`Affected Bucket B items:`, JSON.stringify(bucketBAffected[0]?.items ?? []));
        console.log(`Affected Bucket C items:`, JSON.stringify(bucketCAffected[0]?.items ?? []));
        console.log('====================================');
