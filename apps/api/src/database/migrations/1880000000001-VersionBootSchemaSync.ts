...
/**
 * Moves the ad-hoc DDL that AppService.onApplicationBootstrap used to run
 * (inside a swallowed try/catch!) into a real, versioned migration. Several
 * of these columns — bills.sequence, bills.allocation_type,
 * bills.allocation_config, pos_terminals.account_number — existed ONLY via
 * that boot hook: had it ever failed, every bill read would have crashed
 * with "column does not exist". Every statement is IF NOT EXISTS, so this
 * no-ops on databases the boot hook already patched.
 */
export class VersionBootSchemaSync1880000000001 implements MigrationInterface {
  name = 'VersionBootSchemaSync1880000000001';
  ...
  public async down(): Promise<void> {
    // Additive IF NOT EXISTS statements over pre-existing production schema:
