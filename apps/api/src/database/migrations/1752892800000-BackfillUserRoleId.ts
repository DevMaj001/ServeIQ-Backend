...
export class BackfillUserRoleId1752892800000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        roles.forEach((r: { id: string; name: string }) => roleMap.set(r.name, r.id));
        console.log('Found roles:', Array.from(roleMap.entries()));

        // Verify all expected roles exist
        const expectedRoles = ['Owner', 'Manager', 'Supervisor', 'Waiter', 'Chef', 'Cashier'];
        ...
        for (const [legacyRole, roleName] of Object.entries(roleMapping)) {
            ...
            if (!roleId) {
                console.warn(`Warning: No role ID found for ${roleName} (mapped from ${legacyRole})`);
                continue;
            }
            ...
            if (count > 0) {
                console.log(`Updated ${count} users with legacy role "${legacyRole}" -> role_id ${roleId} (${roleName})`);
            }
        }
        ...
        if (remaining.length > 0) {
            console.warn('WARNING: The following users still have NULL role_id after backfill:');
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        console.log('No-op down migration: role_id backfill is not meaningfully reversible.');
    }
}