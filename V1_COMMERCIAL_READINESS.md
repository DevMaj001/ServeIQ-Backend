...
| H4 | Project-wide lint debt — NEW | Full lint reports ~298 pre-existing eslint errors: mostly `@typescript-eslint/no-unsafe-*` on `any`-typed mocks/`req` across controllers and prettier formatting that was never enforced (no CI existed to block it) | CI lint runs with `continue-on-error: true` so debt is visible without blocking. **Action:** pay down in slices — (a) type mock params in new specs, (b) prettier `--write` file-by-file, (c) eventually flip lint to a hard gate |
