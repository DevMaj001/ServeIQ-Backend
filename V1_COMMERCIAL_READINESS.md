...
| H4 | Project-wide lint debt — NEW | Full lint reports ~298 pre-existing eslint errors: mostly `@typescript-eslint/no-unsafe-*` on `any`-typed mocks/`req` across controllers and prettier formatting that was never enforced (no CI existed to block it) | **SLICE 1 DONE:** All 2451 `prettier/prettier` errors auto-fixed via `eslint --fix` (210 files; build + all 131 unit tests verified green, commit `8e690e3`). Lint debt fell 3,783 → 1,332. **SLICE 2/3 DONE (typing):** `order` module (service/controller/entity/DTOs, `3fd0e2f`) and `ingredient` + `ai` stock-movement enums (`3f3defa`) fully typed — entity status/type fields changed from `string` to their enums (`OrderStatus`, `FulfillmentType`, `StockMovementType`), `req: any` → inline `{ user: {...} }` types, raw `dataSource.query` → `query<T>`, `create(data: any)` → DTO types; surfaced+fixed 2 latent type errors (modifiers `null` vs non-null field; `ai` lowercase stock `type` filters). Build + 131 tests green after each slice; debt now ~1,200. **Remaining:** ~1,200 `no-unsafe-*` — mainly test spec mocks (`bill.calculation.spec` 132, `tab.service.spec` 89, `bill.service.spec` 79, `auth.service.spec` 53) and third-party integration services (`receipt.service`/pdfkit 128, `printer.service`/thermal 88, `subscription.service`/Paystack 53, `admin` 40, `user` 33, `ai` 32). CI lint stays non-blocking until paid down; then flip to hard gate. |
